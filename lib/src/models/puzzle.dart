...
/// Model for a puzzle.
/// {@endtemplate}
class MyPuzzle extends Equatable {
  /// {@macro puzzle}
  const MyPuzzle({required this.tiles});

  /// List of [MyTile]s representing the puzzle's current arrangement.
  final List<MyTile> tiles;
  ...
  MyTile? getTileRelativeToWhitespaceTile(Offset relativeOffset) {
  ...
  /// tile.
  bool isTileMovable(MyTile tile) {
    final whitespaceTile = getWhitespaceTile();
    if (tile == whitespaceTile) {
      return false;
    }
  ...
  bool _isInversion(MyTile a, MyTile b) {
  ...
  MyPuzzle moveTiles(MyTile tile, List<MyTile> tilesToSwap) {
    final whitespaceTile = getWhitespaceTile();
  ...
  MyPuzzle _swapTiles(List<MyTile> tilesToSwap) {
  ...
  MyPuzzle sort() {
    ...
    return MyPuzzle(tiles: sortedTiles);
  }
